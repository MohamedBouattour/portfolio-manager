...
ssh -o StrictHostKeyChecking=no $SSH_KEY root@79.137.14.75 "cd /mnt/main/portfolio && tar -xzf deploy.tar.gz && npm install --omit=dev && pm2 restart ecosystem.config.cjs && rm -f deploy.tar.gz"
